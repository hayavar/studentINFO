...
conn=sqlite3.connect("STDLOGIN.db")
c=conn.cursor()
def create():
    c.execute("CREATE TABLE stuff(USERNAME TEXT,PASSWORD TEXT)")
def creat1():
    c.execute("CREATE TABLE INF(NAME TEXT,USN TEXT,SEMISTER REAL,DEPARTMENT TEXT,GPA REAL,EFFICIENCY REAL,IMP_AREA TEXT)")
def entry():
    for row in range(3):
